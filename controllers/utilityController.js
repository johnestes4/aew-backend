  ...
  try {
    ...
    if (Array.isArray(names[0])) {
      ...
      for (let i = 0; i < names.length; i++) {
        ...
        if (i < names.length - 1) {
          ...
          for (let i2 = i + 1; i2 < names.length; i2++) {
            ...
            var members = [names[i]._id, names[i2]._id].sort();
            var comboID = JSON.stringify(members);
            var found = await Team.findOne({ comboID: comboID });
            if (found) {
              teamMap = testTeam(found, members, teamMap);
      ...
      for (let i = 0; i < singleNames.length; i++) {
        ...
        if (
          (singleNames.length > 1 && i < singleNames.length - 1) ||
          teamMap.size > 0
        ) {
          nameOut = nameOut + ', ';
        }
      ...
      if (teamMap.size > 0) {
        ...
        for (let i = 0; i < teamNames.length; i++) {
          ...
          if (teamNames.length > 1 && i < teamNames.length - 1) {
            nameOut = nameOut + ', ';
          }
        }
      }
